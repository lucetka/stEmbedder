...
##########################################################replace###########################
# ------------------------------------------------------------------
# Step 1: Prepare aligned files
# ------------------------------------------------------------------

PREP_ZIP_MAX_ROWS = 3000  # disable ZIP above this to reduce memory pressure
...
if st.button("PREPARE FILES"):
    try:
        ...
        st.session_state["prepared_base"] = base
        st.session_state["prepared_done"] = True

        # Reset any previous embedding outputs when a new preparation is run
        for k in ["emb_df", "title_df", "final_ref_df", "embed_done"]:
...
# Render Step 1 download buttons OUTSIDE the button-click block
if st.session_state.get("prepared_done", False):
    input_df = st.session_state["prepared_input_df"]
    ref_df = st.session_state["prepared_ref_df"]
    base = st.session_state["prepared_base"]
    ...
    st.download_button(
        ...
        mime="text/csv",
        key="dl_prepared_ref_csv",
    )

    if len(input_df) <= PREP_ZIP_MAX_ROWS:
        zip_bytes = build_zip(
            {
        ...
        st.info(
            f"ZIP disabled for prepared datasets above {PREP_ZIP_MAX_ROWS:,} rows. "
            f"Use the two CSV downloads above."
        )
...
if "prepared_input_df" in st.session_state:
    ...
    if st.session_state.get("embed_done", False):
        ...
        if len(emb_df) <= STEP2_ZIP_MAX_ROWS:
            zip_all = build_zip(
                {
                    f"{base}_input4specter.csv": df_to_csv_bytes(
                        st.session_state["prepared_input_df"], index=False
                    ),
                    f"{base}_index_reference.csv": ref_bytes,
            ...
            st.info(
                f"ZIP disabled for embedding outputs above {STEP2_ZIP_MAX_ROWS:,} rows. "
                f"Use the individual CSV downloads above."
            )
else:
